...
export function ComputeProjectPage({ projectId }: { projectId: string }) {
  ...
  const {
    ...
    error,
    retry,
  } = useGatewayResource(`/v1/projects/${encodeURIComponent(projectId)}`, parse);
  if (loading) return <ComputeLoading />;
  if (error) return <GatewayFeedback error={error} retry={retry} />;
